...
class SonicPipelineWrapper:
    """Wrapper around Sonic pipeline for easier usage."""
    ...
    def preprocess(self, image_path: str, expand_ratio: float = 1.0) -> dict:
        """Detect faces and prepare image for processing."""
    ...
    def crop_image(self, input_image_path: str, output_image_path: str, crop_bbox: tuple):
    ...
    def process(
        ...
        """Process image and audio to generate video."""
        config = self.config
        device = self.device
        pipe = self.pipe
